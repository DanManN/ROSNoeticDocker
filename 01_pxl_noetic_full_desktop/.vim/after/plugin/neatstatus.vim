...
" Inter color separator (left)
if !exists('g:NeatStatusLine_separator_left')  | let g:NeatStatusLine_separator_left = '\uE0B0' | endif
" Inter color separator (right)
if !exists('g:NeatStatusLine_separator_right') | let g:NeatStatusLine_separator_right = '\uE0B2' | endif
...
function! Mode()
	let l:mode = mode()
	if     mode ==# 'n'  | exec 'hi User1 '.g:NeatStatusLine_color_normal  | redraw | return 'NORMAL'
	elseif mode ==# 't'  | exec 'hi User1 '.g:NeatStatusLine_color_normal  | redraw | return 'TERM'
	elseif mode ==# 'i'  | exec 'hi User1 '.g:NeatStatusLine_color_insert  | redraw | return 'INSERT'
	elseif mode ==# 'R'  | exec 'hi User1 '.g:NeatStatusLine_color_replace | redraw | return 'REPLACE'
	elseif mode ==# 'v'  | exec 'hi User1 '.g:NeatStatusLine_color_visual  | redraw | return 'VISUAL'
	elseif mode ==# 'V'  | exec 'hi User1 '.g:NeatStatusLine_color_visual  | redraw | return 'V-LINE'
	elseif mode ==# '' | exec 'hi User1 '.g:NeatStatusLine_color_visual  | redraw | return 'V-BLOCK'
	else                 | return l:mode
	endif
endfunc
...
if has('statusline')
	...
	augroup statusline_update
		...
		autocmd ColorScheme * call SetNeatstatusColorscheme()
		" Make sure the statusbar is reloaded late to pick up servername
		autocmd ColorScheme,VimEnter * call SetStatusLineStyle()
	augroup END
